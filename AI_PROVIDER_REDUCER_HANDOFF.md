# SonicStudio Provider, Reducer, And Route Handoff

This handoff describes the codebase after the provider-thinning pass, the reducer-ownership split, and the explicit route-controller pass that followed.
...
4. `trackNoteActions.ts` handles note toggle, note detail edits, note movement, sample-step edits, and gate or velocity ownership
5. `trackClipPatternActions.ts` handles clip-pattern identity and selection-coupled phrase edits
...
9. `trackPatternActions.ts` is now only a thin coordinator across those four ownership zones
...
2. `/Users/erendiracisneros/Documents/New project/repos/SonicStudio/src/context/editor/reducer/trackNoteActions.ts`: current note-editing reducer gravity well
3. `/Users/erendiracisneros/Documents/New project/repos/SonicStudio/src/context/editor/reducer/trackClipPatternActions.ts`: current phrase-editing reducer gravity well
4. `/Users/erendiracisneros/Documents/New project/repos/SonicStudio/src/components/device-rack/source/DeviceRackSampleCorePanel.tsx`: current rack-source gravity well
...
### 1. Note and clip-pattern ownership are the new reducer gravity wells

The reducer is no longer concentrated in one file, but note editing and clip-pattern editing still hold the densest remaining ownership.

That is acceptable for this pass, but it is the next reducer target.

Likely next split:
1. note toggle and note-event editing
2. sample-step mapping and gate editing
3. clip-pattern identity and phrase ownership
4. any remaining shared transform utilities that still span both modules
