...
export const SettingsSidebar = () => {
  ...
  return (
    <aside className="surface-panel w-[340px] shrink-0 overflow-auto p-4">
      <input
        ref={fileInputRef}
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">Transport controls, session actions, and selected-track settings live here.</p>
        </div>
        <button
      ...
      <div className="mt-4 space-y-4">
        <section className="surface-panel-strong p-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm text-[var(--text-secondary)]">
            <div>
              ...
              <div className="mt-2 text-xl font-semibold text-[var(--text-primary)]">{tracks.length}</div>
            </div>
            <div>
              <div className="section-label">Status</div>
              <div className={`mt-2 text-sm font-medium ${saveStatus === 'error' ? 'text-[var(--danger)]' : saveStatus === 'saving' ? 'text-[var(--warning)]' : 'text-[var(--accent-strong)]'}`}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            ...
            <ActionButton icon={<Download className="h-3.5 w-3.5" />} label="Export JSON" onClick={exportSession} />
          </div>
        </section>
        ...
        <section className="surface-panel-strong p-4">
          ...
          {selectedTrack ? (
            ...
            <div className="mt-4 border border-dashed border-[var(--border-soft)] bg-[rgba(255,255,255,0.02)] px-4 py-6 text-sm text-[var(--text-secondary)]">
              Select a track from the sequencer, piano roll, or mixer to edit its identity and channel settings.
            </div>
          )}
        </section>
...
const ShortcutRow = ({ command, description }: { command: string; description: string }) => (
  <div className="flex items-center justify-between gap-3 border border-[var(--border-soft)] bg-[rgba(255,255,255,0.02)] px-3 py-2">
