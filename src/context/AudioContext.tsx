...
type EditorAction =
  ...
  | { type: 'TOGGLE_STEP'; trackId: string; stepIndex: number; note?: string }
...
const editorReducer = (state: EditorState, action: EditorAction): EditorState => {
  ...
  switch (action.type) {
    ...
    case 'SET_PATTERN_COUNT': {
      const nextProject = resizeProjectTransport(present, action.patternCount, present.transport.stepsPerPattern);
      return commitProject(state, nextProject);
    }

    case 'SET_STEPS_PER_PATTERN': {
      const nextProject = resizeProjectTransport(present, present.transport.patternCount, action.stepsPerPattern);
      return commitProject(state, nextProject);
    }
    ...
    case 'TOGGLE_VOLUME': {
      ...
      const nextProject = updateTrack(present, action.trackId, (track) => (
        nextVolume === track.volume ? track : { ...track, volume: nextVolume }
      ));
    ...
    case 'TOGGLE_PAN': {
      ...
      const nextProject = updateTrack(present, action.trackId, (track) => (
        nextPan === track.pan ? track : { ...track, pan: nextPan }
      ));
...
export const AudioProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <AudioContext.Provider value={{
      ...
      setTrackParams: (trackId, params) => dispatch({ type: 'SET_TRACK_PARAMS', trackId, params }),
